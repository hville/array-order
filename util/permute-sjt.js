 ...
 *
 * Note: only works on sequence since array values are used
 *
 ...
 * @return {ArrayLike}
 */
export default function(N, M=N) {
	const pos = sequence(N),
				dir = Array(M).fill(-1)
	/* will generate N!-1 results */
	return function () {
		let iM = -1 // largest mobile @ iM
		for (let i=0; i<M; ++i) if (
		...
		if (iM<0) {
			...
			swap(pos, iM, jM)
			swap(dir, iM, jM)
			for (let i=0; i<M; ++i) if (pos[i] > pos[jM]) dir[i] = -dir[i]
		}
		return pos
	}
}
